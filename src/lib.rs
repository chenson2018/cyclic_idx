...
//! original data `Vec<u8>`, the type of this data's indicies `usize` and the type we will use
...
//!
//! You may also use a data structure with a different indexing by providing your own
//! implementation. Here is a bit of a contrived example where our indicies are an enum with two
...
//! ```
//!
//! Note that we have not not implemented a custom method of wrapping, as we are still using usize
//! for both `NativeIdx` and `ForeignIdx`.
//!
//! # Non-`usize` NativeIdx
//!
//! Note that it is not a requirement that NativeIdx is `usize` as in our previous examples.
//! Consider the following where we define a type whoose indicies are a subset of isize:
//!
//! ```
//! # use cyclic_idx::{Cyclic,Wrapping};
//! # use std::ops::Index;
